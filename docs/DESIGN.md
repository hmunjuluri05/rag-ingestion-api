...
graph TB
    ...
    subgraph Azure
        ...
        Kafka_Azure[Azure Event Hubs<br/>Kafka]
        Workers_AKS[Ingestion Worker Pods<br/>AKS]
        Storage_Azure[Azure Blob +<br/>Azure Redis]
    end

    subgraph GCP
        API_GKE[FastAPI API<br/>GKE]
        Kafka_GCP[Managed Kafka<br/>GCP]
        Workers_GKE[Ingestion Worker Pods<br/>GKE]
        Storage_GCP[GCS +<br/>Memorystore]
    end

    MongoDB[MongoDB Atlas<br/>Multi-Cloud Vector DB]

    Client -->|REST API| API_AKS
    Client -->|REST API| API_GKE

    API_AKS -->|Publish| Kafka_Azure
    API_GKE -->|Publish| Kafka_GCP

    Kafka_Azure --> Workers_AKS
    Kafka_GCP --> Workers_GKE

    Workers_AKS <--> Storage_Azure
    Workers_GKE <--> Storage_GCP
    ...
    style Kafka_Azure fill:#0078D4,stroke:#004578,stroke-width:2px,color:#fff
    style Kafka_GCP fill:#4285F4,stroke:#1967D2,stroke-width:2px,color:#fff
    ...
    style Storage_Azure fill:#50E6FF,stroke:#0078D4,stroke-width:2px,color:#000
    style Storage_GCP fill:#AECBFA,stroke:#4285F4,stroke-width:2px,color:#000
...
graph TB
    subgraph "Azure Storage"
        Workers_A[Ingestion Worker Pods AKS]
        Blob[(Azure Blob<br/>Storage)]
        Redis_A[(Azure Redis<br/>Cache)]
    end

    subgraph "GCP Storage"
        Workers_G[Ingestion Worker Pods GKE]
        GCS[(Google Cloud<br/>Storage)]
        Memorystore[(GCP Memorystore<br/>Redis)]
    end

    MongoDB[(MongoDB Atlas<br/>Multi-Cloud Vector DB)]

    Workers_A -->|Store text & chunks| Blob
    Workers_A -->|Job status & dedup| Redis_A
    Workers_A -->|Index vectors| MongoDB

    Workers_G -->|Store text & chunks| GCS
    Workers_G -->|Job status & dedup| Memorystore
    Workers_G -->|Index vectors| MongoDB
    ...
    style Redis_A fill:#50E6FF,stroke:#0078D4,stroke-width:2px,color:#000
    style Memorystore fill:#AECBFA,stroke:#4285F4,stroke-width:2px,color:#000
    style MongoDB fill:#4DB33D,stroke:#13AA52,stroke-width:2px,color:#fff
```
...
graph TB
    subgraph Azure ["AZURE CLOUD"]
        ...
        API_Pods_A[FastAPI API]
        Worker_Pods_A[Ingestion Worker Pods]
        Search_Pods_A[Search API<br/>ISOLATED]

        EventHubs_A[Azure Event Hubs<br/>Kafka]
        Blob_A[Azure Blob Storage]
        Redis_A[Azure Redis Cache]

        AKS_Cluster --> API_Pods_A
        AKS_Cluster --> Worker_Pods_A
        AKS_Cluster -.->|Isolated| Search_Pods_A

        API_Pods_A --> EventHubs_A
        Worker_Pods_A --> EventHubs_A
        Worker_Pods_A --> Blob_A
        Worker_Pods_A --> Redis_A
    end

    subgraph External ["EXTERNAL SERVICES"]
        MongoDB_Atlas[MongoDB Atlas<br/>Vector Database]
        Kong[Kong API Gateway<br/>LLM & Embedding APIs]
    end
    ...
    subgraph GCP ["GOOGLE CLOUD PLATFORM"]
        ...
        API_Pods_G[FastAPI API]
        Worker_Pods_G[Ingestion Worker Pods]
        Search_Pods_G[Search API<br/>ISOLATED]

        Kafka_G[GCP Managed Kafka]
        GCS_G[Google Cloud Storage]
        Memorystore_G[GCP Memorystore]
        ...
        API_Pods_G --> Kafka_G
        Worker_Pods_G --> Kafka_G
        Worker_Pods_G --> GCS_G
        Worker_Pods_G --> Memorystore_G
    end

    Worker_Pods_A --> MongoDB_Atlas
    Worker_Pods_A --> Kong
    Worker_Pods_G --> MongoDB_Atlas
    Worker_Pods_G --> Kong
    ...
    style EventHubs_A fill:#0078D4,stroke:#004578,stroke-width:2px,color:#fff
    style Blob_A fill:#0078D4,stroke:#004578,stroke-width:2px,color:#fff
    style Redis_A fill:#0078D4,stroke:#004578,stroke-width:2px,color:#fff

    style Kafka_G fill:#4285F4,stroke:#1967D2,stroke-width:2px,color:#fff
    style GCS_G fill:#4285F4,stroke:#1967D2,stroke-width:2px,color:#fff
    style Memorystore_G fill:#4285F4,stroke:#1967D2,stroke-width:2px,color:#fff

    style MongoDB_Atlas fill:#4DB33D,stroke:#13AA52,stroke-width:3px,color:#fff
    style Kong fill:#1C4D6B,stroke:#003459,stroke-width:3px,color:#fff
```
...
```mermaid
graph TD
    subgraph Azure ["Azure - Event Hubs Kafka"]
        direction LR
        API_Azure[FastAPI API<br/>AKS Cluster]

        Extract_Azure[extract-topic<br/>Partitions: 10]
        Chunk_Azure[chunk-topic<br/>Partitions: 10]
        Dedup_Azure[dedup-topic<br/>Partitions: 10]
        Embed_Azure[embed-topic<br/>Partitions: 20]
        Index_Azure[index-topic<br/>Partitions: 10]

        API_Azure --> Extract_Azure --> Chunk_Azure --> Dedup_Azure --> Embed_Azure --> Index_Azure
    end

    subgraph GCP ["GCP - Managed Kafka (Confluent)"]
        direction LR
        API_GCP[FastAPI API<br/>GKE Cluster]

        Extract_GCP[extract-topic<br/>Partitions: 10]
        Chunk_GCP[chunk-topic<br/>Partitions: 10]
        Dedup_GCP[dedup-topic<br/>Partitions: 10]
        Embed_GCP[embed-topic<br/>Partitions: 20]
        Index_GCP[index-topic<br/>Partitions: 10]

        API_GCP --> Extract_GCP --> Chunk_GCP --> Dedup_GCP --> Embed_GCP --> Index_GCP
    end
    ...
    style Extract_Azure fill:#50E6FF,stroke:#0078D4,stroke-width:2px,color:#000
    style Chunk_Azure fill:#50E6FF,stroke:#0078D4,stroke-width:2px,color:#000
    style Dedup_Azure fill:#50E6FF,stroke:#0078D4,stroke-width:2px,color:#000
    style Embed_Azure fill:#50E6FF,stroke:#0078D4,stroke-width:2px,color:#000
    style Index_Azure fill:#50E6FF,stroke:#0078D4,stroke-width:2px,color:#000

    style Extract_GCP fill:#AECBFA,stroke:#4285F4,stroke-width:2px,color:#000
    style Chunk_GCP fill:#AECBFA,stroke:#4285F4,stroke-width:2px,color:#000
    style Dedup_GCP fill:#AECBFA,stroke:#4285F4,stroke-width:2px,color:#000
    style Embed_GCP fill:#AECBFA,stroke:#4285F4,stroke-width:2px,color:#000
    style Index_GCP fill:#AECBFA,stroke:#4285F4,stroke-width:2px,color:#000
```
...
sequenceDiagram
    ...
    participant Kafka as Kafka (Event Hubs/GCP)
    participant Worker as Worker Pod (AKS/GKE)
    participant Storage as Cloud Storage (Blob/GCS)
    participant MongoDB as MongoDB Atlas
    participant Redis as Redis (Azure/Memorystore)

    Client->>API: POST /api/v2/ingest
    Note right of API: documents, config, cloud: azure/gcp
    API->>Redis: Create job entry
    API->>Kafka: Publish to extract-topic
    API-->>Client: job_id, status: queued

    Kafka->>Worker: Consume message
    Worker->>Storage: Download document (cloud-specific)
    Worker->>Worker: Process (extract/chunk/etc)
    Worker->>Storage: Upload results (cloud-specific)
    Worker->>Redis: Update job progress
    Worker->>Kafka: Publish to next topic

    Note over Worker,Kafka: Repeat for each stage (cloud-agnostic)

    Worker->>MongoDB: Insert vectors with metadata
    Worker->>Redis: Update status: completed
    Worker->>Client: Webhook callback (optional)
